...
var ALL_KEYS = ['best_score', 'total_kills', 'season_score'];
 ...
 * 请求所有 key 的好友云存储数据
 */
function fetchAllFriendData(force) {
    if (isLoading) return;

    var now = Date.now();
    if (!force && lastFetchTime > 0 && (now - lastFetchTime) < FETCH_INTERVAL) {
        drawLeaderboard();
        return;
    }
...
function getFilteredData() {
    var key = currentTab;
    var result = [];
    for (var i = 0; i < allFriendData.length; i++) {
        var item = allFriendData[i];
        var score = 0;
        var hasKey = false;
        if (item.KVDataList) {
            for (var k = 0; k < item.KVDataList.length; k++) {
                if (item.KVDataList[k].key === key) {
        ...
        if (hasKey && (score > 0 || key === 'season_score')) {
            result.push(item);
        }
    }
...
function drawLeaderboard() {
    ...
    if (isLoading && allFriendData.length === 0) {
        ...
        sharedCtx.fillText('加载中...', w / 2, h / 2);
        return;
    }

    var friendData = getFilteredData();
